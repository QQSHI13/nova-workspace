...
### Example: Finding a Modal Bug

❌ **Wrong (grep hunting):**
```
grep -n "modal" file.html
grep -n "hide" file.html
grep -n "display" file.html
```

✅ **Right (structured analysis):**
```
# 1. Read HTML structure
read file_path:index.html limit:100 offset:1

# 2. Find the modal element
# Look for: <div class="modal" id="...">

# 3. Find show/hide logic
# Look for: classList.add('hidden'), style.display, etc.

# 4. Find event handlers
# Look for: button onclick, addEventListener

# 5. Trace the bug
# Modal created → button clicked → handler runs → ...
```
